...
//this is a database class its more convenient than using shared preference
class TinyDB(appContext: Context?) {
    ...
    fun getListObjectShow(key: String?):
            ArrayList<Show> {
        val gson = Gson()
        val objStrings = getListString(key)
        val objects = ArrayList<Show>()
        for (jObjString in objStrings) {
            val value = gson.fromJson(jObjString, Show::class.java)
            objects.add(value)
        }
        return objects
    }
    ...
    fun putListObjectShow(key: String?, objArray: ArrayList<Show>) {
        checkForNullKey(key)
        val gson = Gson()
        val objStrings = ArrayList<String>()
        for (obj in objArray) {
    ...
    fun saveShows(key:String,shows: MutableMap<Int, ArrayList<TvSeries>>)
    {
        checkForNullKey(key)
        val copym = mutableMapOf<Int,ArrayList<TvSeries>>()
        for (i in shows)
        {
            copym.put(i.key,i.value)
        }
        val gson = Gson()
        val json = gson.toJson(copym)
        putString(key,json)
    }

    fun saveTv(key:String,shows: ArrayList<TvSeries>)
    {
        checkForNullKey(key)
        val copym = ArrayList<TvSeries>()
        for (i in shows)
        {
            copym.add(i)
        }
    ...
    fun getShows(key:String):MutableMap<Int,ArrayList<TvSeries>>
    {
        val gson = Gson()
        val json = getString(key)
        val type: Type = object : TypeToken<MutableMap<Int,ArrayList<TvSeries>>>() {}.type

        if (json!=null)
        {
            try {
                return gson.fromJson(json,type)
            }catch (e:java.lang.Exception){return mutableMapOf()}
    ...
    fun getTv(key:String):ArrayList<TvSeries>
    {
        val gson = Gson()
        val json = getString(key)
        val type: Type = object : TypeToken<ArrayList<TvSeries>>() {}.type

        if (json!=null)
        {
            try {
                return gson.fromJson(json,type)
            }catch (e:java.lang.Exception){return ArrayList()}
